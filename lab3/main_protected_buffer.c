...
int main(int argc, char *argv[]){
  ...
  data = malloc(sizeof(int) * (n_consumers+n_producers));
  ...
  for (i=0; i<n_consumers; i++) {
    data[i] = i;
    printf("n_consumers=%u\n",n_consumers);
    pthread_create(tasks + i, NULL, main_consumer, &data[i]);
  }
  for (i=n_consumers; i<n_producers+n_consumers; i++) {
    data[i] = i;
    pthread_create(tasks + i, NULL, main_producer, &data[i]);
  }
